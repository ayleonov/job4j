...
public class ParserVacancies implements AutoCloseable{
    ...
    public void init() {
        ...
        try (InputStream in = ParserVacancies.class.getClassLoader().getResourceAsStream("app2.properties")) {
            ...
            config.load(in);
            PreparedStatement stat = conn.prepareStatement("CREATE TABLE IF NOT EXISTS vacancy(id serial primery key , name varchar(2000), text varchar(2000), link varchar(2000)");
             stat.execute();
